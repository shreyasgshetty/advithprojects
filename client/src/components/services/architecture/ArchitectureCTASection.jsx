...
import { ArrowRight, MessageCircle, Phone, Building2, Layers } from 'lucide-react'
import { WHATSAPP_URL, TEL_URL, PHONE_DISPLAY } from '../../../config/contact'
import { services } from '../../../data/services'

const EXPO = [0.16, 1, 0.3, 1]

export default function ArchitectureCTASection() {
  const shouldReduceMotion = useReducedMotion()
  const [stickyVisible, setStickyVisible] = useState(false)

  const otherServices = services.filter((s) => s.slug !== 'architecture')
    ...
    <>
      ...
      <section className="py-20 lg:py-24 bg-[#F7F7F5] border-b border-[#E7E5E0]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <span className="text-xs font-mono font-semibold uppercase tracking-widest text-[#D97706] mb-2 block">
              Integrated Practice
            </span>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-2 font-light">
              Our architectural design coordinates directly with our civil engineering and interior teams.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {otherServices.map((svc) => {
              const OtherIcon = svc.icon === 'building2' ? Building2 : Layers
                ...
                >
                  <div
                    className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${
                      isConst ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-rose-50 text-rose-600 border border-rose-100'
                    }`}
                  >
                    <OtherIcon className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-bold text-slate-900 mb-1 group-hover:text-[#D97706] transition-colors">
                      {svc.title}
                    </h3>
                    <p className="text-xs text-slate-600 leading-relaxed font-light mb-3 line-clamp-2">
                      {svc.navDescription}
                    </p>
                    <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#D97706] group-hover:gap-2 transition-all">
      ...
      <section className="py-24 lg:py-32 bg-[#0B0F17] text-white relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <motion.div {...anim(0)}>
            <span className="inline-block px-3 py-1 rounded-md bg-slate-900 border border-slate-700/60 text-amber-400 font-mono text-xs uppercase tracking-wider mb-6">
            ...
            <p className="text-slate-300 font-light text-sm sm:text-base leading-relaxed max-w-2xl mx-auto mb-10">
              Share your site location, project requirements, and spatial aspirations with our studio to begin the architectural planning process.
            </p>

            <div className="flex flex-wrap gap-4 justify-center items-center mb-10">
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 px-8 py-4 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm rounded-xl shadow-lg shadow-amber-600/25 transition-all active:scale-[0.98]"
              >
            ...
            <div className="text-xs font-mono text-slate-500">
              Advith Projects · Architecture, Spatial Planning & Design
            </div>
          </motion.div>
        </div>
      </section>
        ...
        {stickyVisible && (
          ...
          >
            <div className="flex items-center gap-3">
              <Link
                to="/contact"
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors"
              >
